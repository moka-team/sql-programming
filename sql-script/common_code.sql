...
show tables;
select * from result_2020 where Respondent=1;
select * from result_2018;
...
CREATE TABLE result_2020 (
    respondent BIGINT NOT NULL,
    main_branch VARCHAR(255),
    ed_level VARCHAR(255),
    dev_type VARCHAR(1000),
    country VARCHAR(255),
    age INT,
    employment VARCHAR(255),
    ethnicity VARCHAR(255),
    gender VARCHAR(255),
    converted_comp_yearly INT,
    database_have_worked_with VARCHAR(1000),
    database_want_to_work_with VARCHAR(1000),
    language_have_worked_with VARCHAR(1000),
    language_want_to_work_with VARCHAR(1000),
    op_sys_professional_use VARCHAR(1000),
    platform_have_worked_with VARCHAR(1000),
    platform_want_to_work_with VARCHAR(1000),
    webframe_have_worked_with VARCHAR(1000),
    webframe_want_to_work_with VARCHAR(1000)
);

select @@GLOBAL.secure_file_priv;

LOAD DATA INFILE '/var/lib/mysql-files/survey_results_public_2020.csv'
INTO TABLE result_2020
FIELDS TERMINATED BY ','
ENCLOSED BY '"'
LINES TERMINATED BY '\n'
IGNORE 1 ROWS
(@Respondent,@MainBranch,@Age,@ConvertedComp,@Country,@DatabaseDesireNextYear,@DatabaseWorkedWith,@DevType,@EdLevel,@Employment,@Ethnicity,@Gender,@LanguageDesireNextYear,@LanguageWorkedWith,@OpSys,@PlatformDesireNextYear,@PlatformWorkedWith,@WebframeDesireNextYear,@WebframeWorkedWith)
SET
    respondent = @Respondent,
	main_branch = CASE WHEN @MainBranch='NA' THEN NULL ELSE @MainBranch END,
	ed_level = CASE WHEN @EdLevel='NA' THEN NULL ELSE @EdLevel END,
	dev_type = CASE WHEN @DevType='NA' THEN NULL ELSE @DevType END,
	country = CASE WHEN @Country='NA' THEN NULL ELSE @Country END,
	age = CASE WHEN @Age='NA' THEN NULL ELSE @Age END,
	employment = CASE WHEN @Employment='NA' THEN NULL ELSE @Employment END,
	ethnicity = CASE WHEN @Ethnicity='NA' THEN NULL ELSE @Ethnicity END,
	gender = CASE WHEN @Gender='NA' THEN NULL ELSE @Gender END,
	converted_comp_yearly = CASE WHEN @ConvertedComp='NA' THEN NULL ELSE @ConvertedComp END,
	database_have_worked_with = CASE WHEN @DatabaseWorkedWith='NA' THEN NULL ELSE @DatabaseWorkedWith END,
	database_want_to_work_with = CASE WHEN @DatabaseDesireNextYear='NA' THEN NULL ELSE @DatabaseDesireNextYear END,
	language_have_worked_with = CASE WHEN @LanguageWorkedWith='NA' THEN NULL ELSE @LanguageWorkedWith END,
	language_want_to_work_with = CASE WHEN @LanguageDesireNextYear='NA' THEN NULL ELSE @LanguageDesireNextYear END,
	op_sys_professional_use = CASE WHEN @OpSys='NA' THEN NULL ELSE @OpSys END,
	platform_have_worked_with = CASE WHEN @PlatformWorkedWith='NA' THEN NULL ELSE @PlatformWorkedWith END,
	platform_want_to_work_with = CASE WHEN @PlatformDesireNextYear='NA' THEN NULL ELSE @PlatformDesireNextYear END,
	webframe_have_worked_with = CASE WHEN @WebframeWorkedWith='NA' THEN NULL ELSE @WebframeWorkedWith END,
	webframe_want_to_work_with = CASE WHEN @WebframeDesireNextYear='NA' THEN NULL ELSE @WebframeDesireNextYear END;


create table result_2020_1NF as select * from result_2020;
drop table result_2020_1NF;

CREATE TABLE result_2020_1NF
AS SELECT DISTINCT respondent
              , TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(main_branch,';',n+1),';',-1)) main_branch
              , TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(ed_level,';',n+1),';',-1)) ed_level
              , TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(dev_type,';',n+1),';',-1)) dev_type
              , TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(country,';',n+1),';',-1)) country
              , age
              , TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(employment,';',n+1),';',-1)) employment
              , TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(ethnicity,';',n+1),';',-1)) ethnicity
              , TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(gender,';',n+1),';',-1)) gender
              , converted_comp_yearly
              , TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(database_have_worked_with,';',n+1),';',-1)) database_have_worked_with
           , TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(database_want_to_work_with,';',n+1),';',-1)) database_want_to_work_with
           , TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(language_have_worked_with,';',n+1),';',-1)) language_have_worked_with
           , TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(language_want_to_work_with,';',n+1),';',-1)) language_want_to_work_with
            , TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(platform_have_worked_with,';',n+1),';',-1)) platform_have_worked_with
           , TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(platform_want_to_work_with,';',n+1),';',-1)) platform_want_to_work_with             
           , TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(webframe_have_worked_with,';',n+1),';',-1)) webframe_have_worked_with
           , TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(webframe_want_to_work_with,';',n+1),';',-1)) webframe_want_to_work_with
           , TRIM(SUBSTRING_INDEX(SUBSTRING_INDEX(op_sys_professional_use,';',n+1),';',-1)) op_sys_professional_use
           FROM numbers
              , result_2020
          ORDER 
             BY respondent;
